...
export default function SetPasswordPage() {
    React.useEffect(() => {
        document.title = `${PAGES.ACCOUNT.SET_PASSWORD.TITLE} | HieuShop`;
    }, []);
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<SetPasswordForm>({
    ...
    const onSubmit = async (data: SetPasswordForm) => {
        try {
            ...
            setTimeout(() => {
                navigate(PAGES.ACCOUNT.INFO.PATH);
            }, 2000);
        } catch (err: any) {
            console.error(err);
            setError(parseApiError(err, 'Thiết lập mật khẩu thất bại. Vui lòng thử lại.'));
        }
    };
            ...
            <div className="flex flex-col items-center justify-center min-h-[70vh] px-4">
                <div className="w-full max-w-md p-8 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800">
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Thiết Lập Mật Khẩu</h1>
                        <p className="mt-2 text-gray-500 dark:text-gray-400 text-sm">
                            Tài khoản của bạn chưa có mật khẩu. Vui lòng thiết lập mật khẩu để tăng cường bảo mật.
                        </p>
                    </div>
                    ...
                    <div className="mt-6 text-center">
                        <Link to={PAGES.ACCOUNT.INFO.PATH} className="inline-flex items-center text-sm font-medium text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors">
