            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 space-y-16">
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                            {categories.map((category) => (
                                ...
                                >
                                    <img src={category.imageUrl} alt={category.name} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-40 transition-opacity" />
                                    <span className="relative z-10 font-medium text-gray-900 dark:text-gray-100 text-center text-lg">{category.name}</span>
                                </Link>
                            ))}
                        </div>
                    )}
                    ...
                    {isLoading ? (
                        ...
                        <div className="flex flex-wrap gap-4 justify-center md:justify-between items-center py-4">
                            {brands.map((brand) => (
                                ...
                                >
                                   <img src={brand.logoUrl} alt={brand.name} className="h-12 object-contain" title={brand.name} />
                                </Link>
                            ))}
                        </div>
                    )}
                ...
                <section>
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Link to={PAGES.PRODUCTS.PATH} className="text-brand-600 hover:text-brand-500 font-medium flex items-center gap-1">
                    ...
                    {isLoading ? (
                         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {[1, 2, 3, 4].map(i => (
                                <div key={i} className="animate-pulse bg-gray-200 dark:bg-gray-800 rounded-lg h-80"></div>
                            ))}
                         </div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
