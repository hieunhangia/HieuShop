...
import { Search, Filter, ArrowUp, ArrowDown, ChevronLeft, ChevronRight, Loader } from 'lucide-react';
...
export default function ProductsPage() {
    ...
    const sortColumn = searchParams.get('sortCol') || PRODUCT_SORT_COLUMN.CREATED_AT;
    const sortDirection = searchParams.get('sortDir') || SORT_DIRECTION.DESC;

    useEffect(() => {
        document.title = `${slug ? 'Danh sách sản phẩm' : PAGES.PRODUCTS.TITLE} | HieuShop`;
    }, [slug]);
    ...
    const toggleSortDirection = () => {
        const newDir = sortDirection === SORT_DIRECTION.ASC ? SORT_DIRECTION.DESC : SORT_DIRECTION.ASC;
        const newParams = new URLSearchParams(searchParams);
             ...
             <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
                ...
                {isLoading ? (
                            ...
                            <div className="flex justify-center items-center gap-2 mt-8">
                                ...
                                <div className="flex items-center gap-1">
                                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                                        .filter(page => page === 1 || page === totalPages || (page >= pageIndex - 1 && page <= pageIndex + 1))
                    ...
                    <div className="text-center py-12">
                        ...
                        <h3 className="mt-2 text-sm font-semibold text-gray-900 dark:text-white">Không tìm thấy sản phẩm</h3>
                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Hãy thử thay đổi bộ lọc hoặc từ khóa tìm kiếm của bạn.</p>
                    </div>
                )}
            </div>
        </MainLayout>
